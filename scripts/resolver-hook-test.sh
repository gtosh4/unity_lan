...
#
# Proves the resolver hook end to end: our `.internal` DNS server + the real `resolver.rs`
# ResolvectlHook point the OS resolver at it via a `~internal` routing domain, so `resolvectl
# query <name>.internal` returns our answer — while global DNS is untouched. Scoped to a throwaway
...
NAME="host-a.alice.lan.internal"
IP="100.64.0.9"
...
# Our real `.internal` resolver, serving one name (bound on the link IP).
"$ENG" dns-serve "$BIND" "$NAME" "$IP" & SRV=$!
sleep 0.4

# The production hook: resolvectl dns <iface> <server> + domain <iface> ~internal.
"$ENG" resolver-install "$IFACE" "$BIND" || { echo "FAIL: resolver-install"; exit 1; }
resolvectl dnssec "$IFACE" no   # `.internal` is unsigned; don't let DNSSEC validation drop it
resolvectl flush-caches

# Positive: our .internal name resolves through the OS resolver to our answer.
GOT="$(resolvectl query --legend=no "$NAME" 2>/dev/null | awk '{print $2; exit}')"
if [ "$GOT" = "$IP" ]; then
  echo "PASS: resolvectl query $NAME -> $GOT (via .internal routing domain)"
else
...
# Sanity: the routing domain is scoped — resolved shows ~internal only on our link.
resolvectl domain "$IFACE" | grep -q 'internal' && echo "PASS: ~internal routing domain on $IFACE" \
