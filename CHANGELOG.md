...
- **No log file grows forever any more.** Every log this writes — the Windows service's, and any
  `log_file` you set on either platform — was a plain append that grew for the life of the install,
  with nothing ever pruning it. They now roll daily and keep a week: long enough to still hold a
  failure you're only now getting round to reporting, bounded so a chronically flapping peer can't
  quietly fill the disk of a box nobody logs into. The name gains the day it covers, so `engine.log`
  is written as `engine.<date>.log`. Linux packaged installs are unaffected either way — the journal
  has always held this, and rotates already.
- **`log_file` now works for the Windows service.** It was read only on the command line the service
  never takes, so setting it did nothing and the service logged beside its executable regardless. A
  new install now keeps its log with the rest of the engine's state, in
  `C:\ProgramData\UnityLAN`, where [troubleshooting](docs/troubleshooting.md) can point at it.
  Upgrading keeps the config you have, so the log stays where it was — still there, still rotated.
