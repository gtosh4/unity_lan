...
Every open port is a service, so there is only ever one list. A port you opened without naming it —
from the command line, or carried over from an older version — is called `port-25565` after its
number, and resolves like any other name. To give it a better one, add a service with the same port
and scope: that renames it in place rather than opening a second copy.
