...
# exactly unitylan-engine.exe / unitylan-gui.exe (the engine matches archive entries on those names;
# the TLS proxy is the engine image under a hidden subcommand, so it needs no entry). `tar`
